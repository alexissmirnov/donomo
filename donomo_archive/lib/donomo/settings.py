...
SQS_VISIBILITY_TIMEOUT = int(query_env('SQS_VISIBILITY_TIMEOUT', 1800))
...
if PAYPAL_TEST:
    ...
    PAYPAL_PRIVATE_CERT = join_and_normalize(DONOMO_PATH, '/etc/paypal/sandbox_donomo_private_key.pem')
    PAYPAL_PUBLIC_CERT = join_and_normalize(DONOMO_PATH, '/etc/paypal/sandbox_donomo_public_cert.pem')
    PAYPAL_CERT = join_and_normalize(DONOMO_PATH, '/etc/paypal/sandbox_paypal_public_cert.pem')
