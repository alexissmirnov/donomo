        ...
        if not self.flag:
            ...
            if self.is_transaction():
                ...
                if self.payment_status != "Completed" and self.payment_status != "Pending":
                    logging.error("Invalid payment_status (%s). " % self.payment_status)
                    self.set_flag("Invalid payment_status (%s). " % self.payment_status)
                if duplicate_txn_id(self):
                    logging.error("Duplicate transaction ID (%s). " % self.txn_id)
                    self.set_flag("Duplicate transaction ID (%s). " % self.txn_id)
                if self.receiver_email != RECEIVER_EMAIL:
                    logging.error("Invalid receiver_email (%s). Need %s " % (self.receiver_email, RECEIVER_EMAIL))
