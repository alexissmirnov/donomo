...
LawnchairSC.DataSource = SC.DataSource.extend({
  ...
  */
			...
			if( record ) {
				...
				if( nestedDataSource ) {
					nestedDataSource.retrieveRecord(
							...
							storeKey, 
							id, 
							that._didRetrieveRecordFromNestedStore);
				}
			}
		});
	},
	 ...
	 * store.
	 * So this datastore is not in position to insert the record into DB.
	 */
	_didRetrieveRecordFromNestedStore: function(store, storeKey, id) {
		//TODO
	},
	...
	createRecord : function(store, storeKey, params) {
		...
		table.save( {
			...
			SC.Logger.log('DatabaseDataSource.createRecord: Wrote %@:%@ to db'
					.fmt(name.table, id));
